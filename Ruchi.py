...
def Api_connect():
    Api_Id = "AIzaSyANQyz4EbQzBe-0dhrp7t33da8RSxMQk1w"
...
client=pymongo.MongoClient("mongodb+srv://ruchishya:aadritireddy@ruchi.okslvyy.mongodb.net/?retryWrites=true&w=majority&appName=ruchi")
...
def videos_table(channel_name_s):
    ...
    df_single_videos_detail= pd.DataFrame(single_videos_detail[0])
...
with st.sidebar:
    st.title(":red[PROJECT - YOUTUBE DATA HARVERSTING AND WAREHOUSING]")
    st.header(":orange[Index]")
    st.caption("Languages Used >>> Python & SQL")
    st.caption("Data Collection through Youtube API")
    st.caption("Database used >>> MongoDB Atlas")
    st.caption("Query Tool used >>> Postgre SQL")
    st.caption("Data Visualisation >>> Streamlit")

st.title(":orange[Youtube Data Collection, Storing and Visualisation]")
channel_id=st.text_input(":red[Enter the Channel ID]")

if st.button(":orange[Collect and Store Data in MongoDB]"):
    ch_ids=[]
    db=client["Youtube_data"]
    coll1=db["channel_details"]
    for ch_data in coll1.find({},{"_id":0,"channel_information":1}):
...
unique_channel=st.selectbox(":red[Select the Channel]",all_channels)

if st.button(":orange[Migrate to SQL]"):
...
question=st.selectbox(":red[Select Question from Drop Box for Information]",("1. What are the names of all the videos and their corresponding channels?",
